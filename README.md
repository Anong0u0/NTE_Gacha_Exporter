...
- Release portable package 內建 assets pack，桌面 GUI 可顯示抽卡項目與卡池圖片
...
Release portable package 內建 assets pack。assets pack 只包含目前內建 maps 引用到的圖片，來源固定為 [Waifus-Grace/NTE_Assets](https://github.com/Waifus-Grace/NTE_Assets) 的釘選 commit。

位置在 portable 目錄的 `app/assets-pack/current`，視為 app resource，不由使用者管理。GUI 只透過內部 `nteasset` protocol 讀取 `assets/*.webp`，不開放任意檔案讀取。
