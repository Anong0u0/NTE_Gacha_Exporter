...
fn portable_entries(prefix: &str, version: &str) -> Vec<(String, Vec<u8>)> {
    vec![
        (
            format!("{prefix}nte-gacha-exporter.exe"),
            b"launcher".to_vec(),
        ),
        (
            format!("{prefix}nte-gacha-exporter-cli.exe"),
            b"cli".to_vec(),
        ),
        (
            format!("{prefix}app/nte-gacha-exporter-desktop.exe"),
            b"desktop".to_vec(),
        ),
        (
            format!("{prefix}app/nte-gacha-exporter-updater.exe"),
            b"updater".to_vec(),
        ),
        (
...
fn create_current_install(root: &Path, version: &str) {
    ...
    std::fs::write(root.join("nte-gacha-exporter.exe"), b"old launcher").unwrap();
    std::fs::write(root.join("app/nte-gacha-exporter-desktop.exe"), b"old app").unwrap();
    std::fs::write(
        root.join("app/nte-gacha-exporter-updater.exe"),
        b"old updater",
    )
    .unwrap();
    std::fs::write(
    ...
    .unwrap();
    std::fs::write(root.join("sidecars/legacy-sidecar.exe"), b"old sidecar").unwrap();
    std::fs::write(
...
fn stage_update_accepts_single_root_package_and_prepares_helper() {
    ...
    write_portable_zip(&archive, "nte-gacha-exporter-0.2.0/", "0.2.0");
...
fn stage_update_rejects_zip_slip_and_data_files() {
    ...
    let mut entries = portable_entries("nte-gacha-exporter-0.2.0/", "0.2.0");
    entries.push((
        "nte-gacha-exporter-0.2.0/data/profiles/default/records.json".to_string(),
...
#[test]
fn stage_update_rejects_legacy_sidecars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join("install");
    std::fs::create_dir_all(&root).unwrap();
    ...
    let mut entries = portable_entries("nte-gacha-exporter-0.2.0/", "0.2.0");
    entries.push((
        "nte-gacha-exporter-0.2.0/sidecars/legacy-sidecar.exe".to_string(),
        b"legacy sidecar".to_vec(),
    ));
    let borrowed = entries
        .iter()
        .map(|(name, bytes)| (name.as_str(), bytes.as_slice()))
...
fn stage_update_rejects_release_version_mismatch() {
    ...
    write_portable_zip(&archive, "nte-gacha-exporter-0.2.0/", "0.1.0");
    let package = package_for_archive(&archive, "0.2.0");
...
fn apply_staged_update_replaces_release_and_preserves_data() {
    ...
    assert_eq!(
        std::fs::read(root.join("nte-gacha-exporter.exe")).unwrap(),
        b"launcher"
    );
    assert_eq!(
