...
fn build_gacha_rules(
        ...
        .any(|(pool_id, row)| pool_id.starts_with("ForkLottery_") && row.is_object())
    {
        ...
        let notes = if source_is_exact {
            vec!["Fork S-class pickup rate is backed by gold drop sequence weights in the asset dump."]
        } else {
            vec!["Fallback numeric rule follows current desktop behavior when structured values are absent."]
        };
        rules.insert(
