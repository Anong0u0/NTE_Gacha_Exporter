...
use nte_automation::{run_auto_page, AutoPageOptions, AutoPageStatus};
use nte_gui_core::{
    available_locales, build_asset_maps, build_capture_document, candidate_ports, capture_doctor,
    capture_live, find_assets_root, find_process_pid, is_admin, read_raw_capture, CaptureOptions,
    CaptureProgress, CaptureRecordBuilder, JsonStore, ParsedRow,
};
...
#[derive(Parser)]
#[command(name = "nte-gacha-cli", about = "NTE gacha history exporter runtime")]
struct Cli {
...
fn default_maps_output_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../nte_gui_core/resources/maps")
}
