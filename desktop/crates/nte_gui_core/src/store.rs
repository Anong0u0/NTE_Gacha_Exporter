...
use crate::analysis::{dashboard_overview, list_records, pool_kind_detail, record_filter_options};
use crate::export::{export_csv, export_public_json};
...
use crate::public_json::parse_public_document;
...
impl JsonStore {
    ...
    fn read_backup_snapshot(&self, path: &Path) -> Result<BackupSnapshot, GuiError> {
        ...
        if manifest.schema != "nte-gacha-data-backup" || manifest.schema_version != 1 {
            return Err(GuiError::InvalidBackup(
                "manifest schema must be nte-gacha-data-backup v1".to_string(),
            ));
        }
